(define-module (sph filesystem tag))

(use-modules (rnrs sorting) (sph)
  (sph alist) (sph filesystem) (sph io) (sph list) (sph other) (sph string) (srfi srfi-1))

(export tag-add tag-default-config
  tag-group tag-group-from-search tag-name->tags-and-extensions& tag-remove tag-replace tag-sort)
...
(define tag-default-config
  (alist-q extensions (get-mime-extensions-cached) delimiter #\. ignore-extensions? #f))
      ...
      (let (name-old (basename path))
        (tag-name->tags-and-extensions& name-old
          (l (tags-old extensions)
            (let (name-new (tag-create-name (union tags-old tags-new) extensions #:config config))
              (if (not (string-equal? name-old name-new))
                (rename-file path (get-unique-path (string-append (dirname path) "/" name-new)))))))))))
            ...
            (if (not-null? tags-old)
              (let*
                ...
                (let (name-new (tag-create-name tags extensions #:config config))
                  (if (not (string-equal? name-old name-new))
                    (rename-file path
                      (get-unique-path (string-append (dirname path) "/" name-new)))))))))))))
